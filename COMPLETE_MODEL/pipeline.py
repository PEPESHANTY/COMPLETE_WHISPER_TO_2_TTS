...
from modules_tts import synthesize_to_wav  # Piper helper (existing)

load_dotenv()

HAVE_VITS = False
try:
    from modules_vits_tts import synthesize_vits  # returns WAV bytes
    HAVE_VITS = True
except Exception:
    HAVE_VITS = False
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Mic → Whisper → GPT-4o-mini → Piper TTS (+ optional VITS)")
    p.add_argument("--whisper-endpoint",
                   default=os.getenv("WHISPER_ENDPOINT", "http://127.0.0.1:9000/transcribe"))
    p.add_argument("--piper-endpoint",
                   default=os.getenv("PIPER_TTS_API", "http://127.0.0.1:5000/tts"))
...
def main():
    ...
    audio_vits_dir  = audio_dir / "vits"         # ← VITS subfolder

    # make sure all dirs exist (including VITS)
    for d in (transcripts_dir, answers_dir, audio_dir, audio_vits_dir):
        d.mkdir(parents=True, exist_ok=True)

    # 1) capture + build transcript
    ...
    # 4) Piper TTS → audio/
    wav_out_piper = audio_dir / f"{args.prefix}_{ts}_piper.wav"
    synthesize_to_wav(answer, lang, str(wav_out_piper))
    print(f"[TTS|Piper] Wrote WAV → {wav_out_piper}")

    # 5) VITS TTS → audio/vits/
    if HAVE_VITS and os.getenv("VITS_TTS_API"):
        try:
            vits_bytes = synthesize_vits(answer)  # returns wav bytes
            wav_out_vits = audio_vits_dir / f"{args.prefix}_{ts}_vits.wav"
            with open(wav_out_vits, "wb") as f:
                f.write(vits_bytes)
            print(f"[TTS|VITS] Wrote WAV → {wav_out_vits}")
        except Exception as e:
            print(f"[TTS|VITS] Skipped due to error: {e}")
    else:
        print("[TTS|VITS] Skipped (module or VITS_TTS_API not available).")
